...
function renderBasecoatSelect(container, { name, labels, order, defaultValue }) {
...
function renderIconRadioGroup(container, { name, labels, icons, order, defaultValue }) {
...
function clampInnerRimWidthMm(mm) {
  return Math.min(INNER_RIM_WIDTH_MAX, Math.max(INNER_RIM_WIDTH_MIN, Math.round(mm)));
}
...
function persistSavedCalculations() {
  localStorage.setItem(STORAGE_KEY, serializeSavedCalculations(savedCalculations));
}
...
function applyFormInputs(inputs) {
  ...
  configureWidthSlider(frontWidthEl, frontWidthValueEl, inputs.rideStyle, inputs.frontWidthMm);
  configureWidthSlider(rearWidthEl, rearWidthValueEl, inputs.rideStyle, inputs.rearWidthMm);
}
...
function renderResults(results) {
  ...
  document.getElementById("frontBar").textContent = results.frontBar.toFixed(2) + " бар";
  document.getElementById("rearBar").textContent = results.rearBar.toFixed(2) + " бар";
...
function saveCurrentCalculation() {
  ...
  try {
    if (editingId) {
      ...
      savedCalculations = createSavedCalculation(savedCalculations, name, inputs, lastResults);
    }
