...
test("tireWidthFromFormToMm: MTB — дюймы, крупные числа как мм", () => {
	...
	assert.equal(
		tireWidthFromFormToMm("16", RIDE_STYLE.TRAIL_MTB),
		16,
		">15 — мм",
	);
});
...
test("snapTireWidthMm: привязка к сетке и смена стиля", () => {
	...
	assert.equal(
		snapTireWidthMm(
			tireWidthStepToMm(2.5, RIDE_STYLE.XCOUNTRY_MTB),
			RIDE_STYLE.XCOUNTRY_MTB,
		),
		64,
	);
	assert.equal(
...
test("shouldUsePressureBandWarning: только прямой борт", () => {
	assert.equal(
		shouldUsePressureBandWarning(RIM_TYPE.TUBELESS_STRAIGHT_SIDE),
		true,
	);
	assert.equal(shouldUsePressureBandWarning(RIM_TYPE.TUBELESS_CROCHET), false);
});
