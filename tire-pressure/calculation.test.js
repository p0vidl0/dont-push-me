...
test("calculateWheelPressures: шоссе 28 мм, перед и зад", () => {
	...
	assert.deepEqual(
		result.warnings,
		[],
		"без предупреждений при нормальном давлении",
	);
});
...
test("calculateWheelPressures: более узкая покрышка — выше давление", () => {
	const narrow = calculateWheelPressures({
		...roadInputs,
		frontWidthMm: 25,
		rearWidthMm: 25,
	});
	const wide = calculateWheelPressures({
		...roadInputs,
		frontWidthMm: 35,
		rearWidthMm: 35,
	});
...
test("calculateWheelPressures: предупреждение при превышении порога полосы", () => {
	...
	assert.equal(
		...
		0,
		"нормальное давление — без предупреждений",
	);
